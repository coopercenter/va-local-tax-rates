...
section_vars   <- c("utl_cie_tel",
                    "utl_cie_water")

section_tbl <- tax_rates %>% select(all_of(reference_vars),
                                    all_of(section_vars))
```
...
section_tbl %>%
  mutate(has_tel_tax    = !is.na(utl_cie_tel),
         has_water_tax  = !is.na(utl_cie_water)) %>%
...
table_11_1_vars <- c("ExternalDataReference", section_vars)
