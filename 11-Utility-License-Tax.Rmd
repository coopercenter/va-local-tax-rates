...
```{r code to make tables 11, message=FALSE, echo=FALSE}
source("prep_data.R")
...
tax_rates %>% 
  ...
  mutate(across(.cols = section_vars, ~ ifelse(.==0, NA, .)))   %>%
...
table_11_1 <- section_tbl %>% select("ExternalDataReference", section_vars)
