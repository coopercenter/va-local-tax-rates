...
tax_rates %>% 
  ...
  mutate(has_RCC_tipping_fees = case_when(!(has_RCC_tipping_fees) ~ FALSE,
                                          ...
                                          TRUE ~ TRUE)) %>%
  mutate(has_RCC_rec_serv_type = (RCC_rec_serv_type %in% c("Contracted","Directly"))) %>%
...
section_tbl %>% 
  ...
  select(ExternalDataReference, RCC_tipping_fees) -> table_20_2
...
section_tbl %>% 
  ...
  select(ExternalDataReference, RCC_rec_serv_type, RCC_rec_charge_fee) -> table_20_3
