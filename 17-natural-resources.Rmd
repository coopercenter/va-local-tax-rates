...
tax_rates %>% 
  ...
  mutate(across(.cols = section_vars, ~ ifelse(.==0, NA, .)))   %>%             # Convert zero values to NA's
  mutate(across(.cols = section_vars, ~ !is.na(.), .names = "has_{.col}")) %>%
...
table_17_1 <- section_tbl %>% select("ExternalDataReference", section_vars)
